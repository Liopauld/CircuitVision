...
**Escrow model:** the buyer's funds stay in `reservedBalance` for the entire fulfilment and are only released to the seller when the buyer marks the order `completed` (the `completed` side effect calls `settlePayment`). `verify_payment` merely acknowledges the buyer paid into escrow — it moves no money. A dispute (raisable from `payment_verified`/`preparing`/`ready`) therefore resolves against *still-reserved* funds: `disputeController` splits the escrow with `releaseReserved` (buyer's refunded portion) + `settlePayment` (seller's portion), so no clawbacks are needed. (`walletService.refundSettledPayment` is retained but unused under this model.)
